...
from workout_api.atleta.models import AtetlaModel
...
async def post(
    ...
    if not categoria:
        raise HTTPException(
            status_code=status.HTTP_400_BAD_REQUEST,
            detail=f'CT {centro_treinamento_nome} não encontrado.'
        )
    try:
        atleta_out = AtletaOut(id=uuid4(),created_at=datetime.utcnow(), **atleta_in.model_dump())
        atleta_model = AtetlaModel(**atleta_out.model_dump(exclude={'categoria', 'centro_treinamento'}))
        atleta_model.categoria_id = categoria.pk_id
        atleta_model.centro_treinamento_nome_id = centro_treinamento_nome.pk_id
        ...
        raise HTTPException(
            ...
            detail='Houve um erro durante a inserção de dados.'
        )
...
async def query(
   ...
   atletas: list[AtletaOut] = (await db_session.execute(select(AtetlaModel))).scalars().all()
#   breakpoint()
...
async def get(id: UUID4, db_session: DatabaseDependency) -> AtletaOut: 
    atleta: AtletaOut = (
      await db_session.execute(select(AtetlaModel).filter_by(id=id))
    ).scalars().first()
